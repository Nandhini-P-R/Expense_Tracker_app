...
def generate_pdf(rows, user_id):
    ...
    cursor.execute("SELECT * FROM highest_spending_category WHERE userid = %s", (user_id,))
...
def login_page():
    ...
    if login_submit:
        ...
        if user:
            ...
            db = connect_to_db()
            cursor = db.cursor()
            cursor.execute("SELECT income_amount FROM income WHERE user_id=%s", (st.session_state.userid,))
    ...
    if 'temp_email' in st.session_state:
        ...
        if st.button("Submit New Password"):
            if new_password and new_password == confirm_password:
                db = connect_to_db()
                cursor = db.cursor()
                cursor.execute("UPDATE users SET password=%s WHERE email=%s", (new_password, st.session_state.temp_email))
                db.commit()
                db.close()
...
def update_income():
    ...
    if st.button("Update Income"):
        try:
            conn = connect_to_db()  
            cursor = conn.cursor()
            cursor.execute("UPDATE income SET income_amount = %s WHERE user_id = %s", 
...
def view_expense_page():
    ...
    with col1:
        if st.button("Bar Chart"):
            db = connect_to_db()
            cursor = db.cursor()
            cursor.execute("SELECT category, SUM(amount) FROM expenses WHERE userid=%s GROUP BY category", (st.session_state.userid,))
